...
class Dense():
    ...
    def setParam(self,shape,lr=0):
        ...
        if len(input_shape) == 1:
            ...
            self.bias = np.random.randn(self.units, input_shape[1])
        else:
            raise Exception('参数类型错误')
        self.weights = np.random.randn(self.units, input_shape[0]) / np.sqrt(input_shape[0])

        self.lr = lr
        print('init {0} weights {1} bias {2}'.format(self.name, self.weights.shape, self.bias.shape))
